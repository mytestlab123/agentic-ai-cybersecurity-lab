...
1. `01-three-source-navigation.png` — unified ECR, S3, and EC2 navigation.
...
2. `02-ecr-clean-scan.png` — ECR / clean Inspector review. The viewer should
   see Amazon Inspector evidence, a clean/compliant result, and the governance
   timeline saying no remediation proposal, human decision, or action is
   required. This followed **Scan ECR image**; **Reopen Finding** is visible
   for a separate repeatable demo and was not clicked. A future digest
   promotion remains a separate human approval action.
3. `03-s3-config-finding.png` — S3 / configuration-risk review.
...
4. `04-ec2-imdsv2-finding.png` — fixed EC2 / IMDSv2 review. The
