...
bucket=${bucket:0:63}
bucket_two="${bucket}-02"
bucket_three="${bucket}-03"
...
export SECCOP_S3_PROTECTED_BUCKETS="${bucket_two},${bucket_three}"
...
cleanup() {
  ...
  if ! $cleanup_done; then
    "$repo_dir/.venv/bin/python" "$repo_dir/scripts/issue47_s3_compliance.py" cleanup --profile "$profile" --region "$region" --bucket "$bucket" --extra-bucket "$bucket_two" --extra-bucket "$bucket_three" >"$evidence_root/cleanup.json" || true
  fi
}
...
"$repo_dir/.venv/bin/python" "$repo_dir/scripts/issue47_s3_compliance.py" create --profile "$profile" --region "$region" --bucket "$bucket_two" --protected >"$evidence_root/create-02.json"
"$repo_dir/.venv/bin/python" "$repo_dir/scripts/issue47_s3_compliance.py" create --profile "$profile" --region "$region" --bucket "$bucket_three" --protected >"$evidence_root/create-03.json"
...
jq -e '.result.bucket_status == [{"label":"Finance reports","state":"ACTION REQUIRED"},{"label":"Audit logs","state":"PROTECTED"},{"label":"Application backups","state":"PROTECTED"}]' "$evidence_root/api-before.json" >/dev/null
...
curl --fail --silent --show-error -X POST -H 'Content-Type: application/json' --data '{"confirm":true}' "http://127.0.0.1:${api_port}/api/demo/reset" >"$evidence_root/api-reset.json"
jq -e '.result.reason_code == "SECCOP_S3_RESET_READY"' "$evidence_root/api-reset.json" >/dev/null
curl --fail --silent --show-error -X POST -H 'Content-Type: application/json' --data '{"mode":"DEMO"}' "http://127.0.0.1:${api_port}/api/scan" >"$evidence_root/api-reset-scan.json"
jq -e '.result.reason_code == "SECCOP_S3_NON_COMPLIANT"' "$evidence_root/api-reset-scan.json" >/dev/null
...
"$repo_dir/.venv/bin/python" "$repo_dir/scripts/issue47_s3_compliance.py" cleanup --profile "$profile" --region "$region" --bucket "$bucket" --extra-bucket "$bucket_two" --extra-bucket "$bucket_three" >"$evidence_root/cleanup.json"
cleanup_done=true
for cleanup_bucket in "$bucket" "$bucket_two" "$bucket_three"; do
  if aws --profile "$profile" --region "$region" s3api head-bucket --bucket "$cleanup_bucket" >>"$evidence_root/cleanup-head.json" 2>>"$evidence_root/cleanup-head.stderr"; then
    printf 'cleanup verification failed: a demo bucket still exists\n' >&2
    exit 1
  fi
done
