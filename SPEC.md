...
`COMPLIANT`. The fixed LAB_01 GUI always shows **Scan EC2 compliance** and
**Reopen Finding**. Reopen explicitly confirms IMDSv1 and fresh Config
`NON_COMPLIANT`; it is idempotent with `FINDING_ALREADY_OPEN` and no
mutation/wait when already open. The CLI reopen command remains the operator
fallback.
